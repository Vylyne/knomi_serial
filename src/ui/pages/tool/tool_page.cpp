...
lv_obj_t *init(lv_obj_t *parent, const printer::State &state) {
  ...
  // Emptied at birth. A fresh LVGL label says "Text", and this is the one
  // widget here whose cache cannot hold an impossible sentinel - an unset
  // filament type is "", which is also what _forget leaves behind, so the
  // update below matches and never writes. The placeholder would then sit in
  // the pool until a spool was named.
  lv_label_set_text(_material, "");
