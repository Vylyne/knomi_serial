...
class Knomi_Serial:
    ...
    def _handle_connect(self):
        try:
            ...
            # self.serial stays None, module runs in no-op mode
    ...
    def _handle_update(self, eventtime):
        try:
            ...
            logging.warning(f"{self.name}: Update error: {e}")
        return eventtime + _SEND_PERIOD

    def _handle_read(self, eventtime):
        if not self.serial or not self.serial.is_open:
            return eventtime + _RECV_PERIOD  # was: bare return (bug)
    ...
    def _send_state(self, state):
        ...
        try:
            ...
            logging.warning(f"{self.name}: Lost connection: {e}")
            try:
                self.serial.close()
            except Exception:
                pass
